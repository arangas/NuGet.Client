...
@"{
    ...
    ""frameworks"": {
        ...
        [Fact]
        public async Task NominateProjectAsync_WithMultiplePackageVersions_Throws()
        {
            const string projectJson = @"{
    ""version"": ""1.2.0-beta1"",
    ""frameworks"": {
        ""netcoreapp1.0"": { }
            ...
            // Act & Assert
            var ex = await Assert.ThrowsAsync<InvalidOperationException>(() => CaptureNominateResultAsync(projectFullPath, pri));
